...
[§FS-005-dispatch](../functional-spec/FS-005-dispatch.md#fs-005-dispatch-what-ephor-watches-it-can-hand-to-an-agent-runtime) lead: the plan language, the runner, and the read-back are
...
as prose and the identifiers as metadata ([§FS-005-dispatch.2](../functional-spec/FS-005-dispatch.md#2-the-ticket-carries-what-ephor-knows-not-a-link-to-it), .8), id
...
([§FS-005-dispatch.6](../functional-spec/FS-005-dispatch.md#6-dispatch-is-offered-where-it-would-work-and-refuses-where-it-would-not)). Running: the invocation of the bound runner as a
...
([§FS-005-dispatch.14](../functional-spec/FS-005-dispatch.md#14-who-does-the-work-is-chosen-and-defaulted-per-project)) — each hand read from the binding's own registry of
...
because the choice binds in one of two spellings ([§FS-005-dispatch.14](../functional-spec/FS-005-dispatch.md#14-who-does-the-work-is-chosen-and-defaulted-per-project)): a
...
artifacts and never from a process table ([§FS-005-dispatch.15](../functional-spec/FS-005-dispatch.md#15-every-operation-is-visible-in-one-place)) — the
...
runner's own refusal lifted from what it printed ([§FS-005-dispatch.16](../functional-spec/FS-005-dispatch.md#16-work-that-should-not-go-on-is-cancelled-and-the-plan-says-so),
...
ticket's execution line takes ([§FS-005-dispatch.14](../functional-spec/FS-005-dispatch.md#14-who-does-the-work-is-chosen-and-defaulted-per-project)).

Detaching and attaching: the run started beneath the screen and the surface
put on it afterwards ([§FS-005-dispatch.20](../functional-spec/FS-005-dispatch.md#20-a-run-of-the-runtime-starts-beneath-the-screen-and-is-watched-by-attaching)). The detached invocation is the
...
runner's own words and only ever shown ([§FS-005-dispatch.20](../functional-spec/FS-005-dispatch.md#20-a-run-of-the-runtime-starts-beneath-the-screen-and-is-watched-by-attaching)), as the release
...
dispatched and fingerprints, never work state ([§FS-005-dispatch.4](../functional-spec/FS-005-dispatch.md#4-the-ledger-is-ephors-record-and-never-the-truth-about-the-work)).
...
refuses with the configured runner named ([§FS-006-project-interface.10](../functional-spec/FS-006-project-interface.md#10-capability-rung-by-rung),
...
the same rung's own sentence ([§FS-005-dispatch.14](../functional-spec/FS-005-dispatch.md#14-who-does-the-work-is-chosen-and-defaulted-per-project)): who can be asked is
...
binding's to say ([§FS-005-dispatch.19](../functional-spec/FS-005-dispatch.md#19-a-workflow-the-runtime-offers-is-an-action-and-its-inputs-are-answered-here)) — and an entry naming one is refused
...
board is then the refresh row alone ([§FS-005-dispatch.15](../functional-spec/FS-005-dispatch.md#15-every-operation-is-visible-in-one-place)) — an operation is
