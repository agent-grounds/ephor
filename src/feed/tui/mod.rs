...
pub(crate) enum Action {
    ...
    ToggleUnread,
    Refresh,
    /// Ask for the burn reading again, under whatever window and grouping the
    /// page now wants (§FS-013-burn.8). The page holds the two choices; the
    /// reading is the API's, and neither is computed while drawing.
    ReadBurn,
    /// Read the transcripts again before the store would have gone stale
    /// (§FS-013-burn.8). Off the draw path, in the shell, like every other
    /// move.
    RescanBurn,
    SetMessage(String),
}
...
enum Screen {
    ...
    /// The burn page, watch-only (§FS-013-burn.8).
    Burn(BurnScreen),
}
...
/// How often the interface reads the burn store while the page over it is
/// open (§FS-013-burn.8). Slower than the work tick because a scan stats
/// every transcript this machine has, and five-minute buckets do not move
/// faster than this anyway.
const BURN_TICK: Duration = Duration::from_secs(10);
...
struct App {
    ...
    /// When the burn store was last read from the tick (§FS-013-burn.8).
    burn_ticked_at: std::time::Instant,
...
impl App {
    fn load(config: &StatusConfig, scope: &crate::scope::Projects) -> Result<Self> {
        ...
        let mut app = App {
            ...
            work_seen: None,
            burn_ticked_at: std::time::Instant::now(),
            jobs: Vec::new(),
            message: String::new(),
        };
    ...
    fn event_loop(
        ...
        loop {
            ...
            // The burn page opens the same way and closes back the same way
            // (§FS-013-burn.8).
            if key.code == KeyCode::Char('$') && !inside {
                self.toggle_burn();
                continue;
            ...
            let action = match &mut self.screen {
                ...
                Screen::Burn(page) => page.handle_key(key.code),
            };
        ...
        match action {
            ...
            Action::Back => {
                ...
                self.screen = match self.saved.take() {
                    Some(previous)
                        if matches!(self.screen, Screen::Operations(_) | Screen::Burn(_)) =>
                    {
                        previous
                    }
                    _ => Screen::Navigator,
            ...
            // The page changed what it is asking for; the reading is the
            // API's and is taken here, off the draw path (§FS-013-burn.8).
            Action::ReadBurn => {
                self.reload_burn();
            }
            Action::RescanBurn => {
                self.ctx.burn_refresh(true);
                self.burn_ticked_at = std::time::Instant::now();
                self.reload_burn();
                self.message = "Read the transcripts again".to_string();
            }
    ...
    fn toggle_operations(&mut self) {
        ...
        let previous = std::mem::replace(&mut self.screen, board);
        // The two watch-only pages are one modal layer: opening either over
        // the other replaces it and keeps the screen the pair opened over,
        // rather than burying it under a slot that only holds one
        // (§FS-005-dispatch.15).
        if !matches!(previous, Screen::Burn(_)) {
    ...
    fn tick(&mut self) -> bool {
        ...
        // The burn store, where the page that reads it is open
        // (§FS-013-burn.8). First, and on its own clock, because it is the
        // only thing here that reads outside the work roots.
        let burned = self.tick_burn();
        ...
        let shown = match &self.screen {
            ...
            // but a job that ended already has (§FS-005-dispatch.17), and so
            // has a burn page that took a fresh reading.
            _ => return jobs_moved || burned,
        };
    ...
    fn draw(&mut self, frame: &mut ratatui::Frame) {
        ...
        match &mut self.screen {
            ...
            Screen::Burn(page) => page.draw(frame, body_area),
        }
        ...
        // on a row of answers depends on the row (§FS-004-quick-actions.2).
            ...
            match &self.screen {
                ...
                Screen::Burn(page) => page.footer().to_string(),
            }
        };
        frame.render_widget(
